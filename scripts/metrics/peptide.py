...
def main(args):
    ...
    print(f"Loaded {len(lines)} result lines from {args.results}")
    id2items = {}
    for line in lines:
    ...
    print(f"Processing {len(id2items)} unique IDs with {sum(len(items) for items in id2items.values())} total items")
    ...
    # ADD THIS:
    print(f"Submitting {len(id2items)} jobs to Ray...")
    futures = [pipeline_eval.remote((item, args.antibody)) for item in id2items.values()]
    print("All jobs submitted, starting processing...")
    
    ref_metrics, metrics = [], []
    from tqdm import tqdm
    pbar = tqdm(total=len(id2items), desc="Processing items")
    ...
    while len(futures) > 0:
        ...
        for done_id in done_ids:
            ...
            # REPLACE the print_log with:
            pbar.set_postfix_str(f"Last: {_id}")
            pbar.update(1)
    ...
    print(f"Calculating dihedral angles for {len(pdbs)} PDB files...")
    dist = dihedral_distribution(pdbs, all_selected_residues=selected_residues, num_cpus=args.num_workers)
    print("Dihedral angle calculation completed")
