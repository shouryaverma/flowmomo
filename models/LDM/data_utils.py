...
def get_valid_block_name(block_S, default_name='UNK'):
    """
    Get a valid block name for PDB format from vocabulary index.
    
    Args:
        block_S: Block index from vocabulary
        default_name: Default name to use if validation fails
        
    Returns:
        Valid block name (max 3 alphanumeric characters)
    """
    if block_S is None:
        return default_name
    
    try:
        block_name = VOCAB.idx_to_abrv(block_S)
    except:
        return default_name
    
    # Handle case where vocabulary returns SMILES or invalid names
    if block_name is None:
        return default_name
    
    # If it's a SMILES string or too long, create a hash-based short name
    if len(block_name) > 3 or not block_name.replace('_', '').isalnum():
        # For SMILES strings, create a short hash-based name
        if any(char in block_name for char in ['=', '(', ')', '[', ']', '#', '1', '2', '3', '4', '5', '6', '7', '8', '9', '0']):
            # This looks like a SMILES string
            hash_val = abs(hash(block_name)) % 999
            return f"X{hash_val:02d}"[:3]  # X01, X02, etc.
        else:
            # Other invalid names, try to clean them up
            cleaned = ''.join(c for c in block_name if c.isalnum())[:3]
            if len(cleaned) >= 1:
...
class OverwriteTask:
    ...
    def get_overwritten_results(
        ...
        for i, index in enumerate(overwrite_indexes):
            ...
            # Use improved block name validation
            block_name = get_valid_block_name(block_S, 'UNK')
            ...
            if block_bonds is None or is_standard_aa(block_name): # use canonical order
                try:
                    ...
                    canonical_order = []  # Fallback if vocabulary lookup fails
            else:
            ...
            if block_bonds is None: 
                try:
                    ...
                    block_bonds = []  # Fallback if vocabulary lookup fails
            else: 
            ...
            if not is_standard_aa(block_name):
                try:
                    ...
                    # Skip if numerical index conversion fails
                    pass
            ...
            for atom_idx1, atom_idx2, prob, bond_type in bond_tuples:
                ...
                if valence_check(gen_mol, atom_idx1, atom_idx2, rdkit_bond) and cycle_check(gen_mol.GetMol(), atom_idx1, atom_idx2, bond_type_to_rdkit(bond_type)): # and sp2_check(gen_mol.GetMol(), atom_idx1, atom_idx2, all_atom_coords):
                    ...
                    try:
                        ...
                        # Skip if explicit bond addition fails
                        continue
        ...
        try:
            ...
            print("WARNING: Failed to generate SMILES, using empty string")
            smiles = ""
