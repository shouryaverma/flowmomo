...
def _contact_preservation_loss(X_gt, X_t, t, batch_ids, block_ids, generate_mask, contact_th=8.0, t_th=0.25):
    """Preserve important residue-residue contacts"""
    with torch.no_grad():
        # Get block centers
        block_centers_gt = scatter_mean(X_gt, block_ids, dim=0, dim_size=block_ids.max() + 1)
        num_blocks = block_centers_gt.shape[0]
        
        if num_blocks < 2:
            return 0
        
        # Create all pairs of blocks manually to avoid fully_connect_edges issues
        block_indices = torch.arange(num_blocks, device=X_gt.device)
        row = block_indices.unsqueeze(1).repeat(1, num_blocks).flatten()
        col = block_indices.unsqueeze(0).repeat(num_blocks, 1).flatten()
        
        # Remove self-connections and keep only upper triangle
        mask = row < col
        row, col = row[mask], col[mask]
        
        # Only consider contacts involving generated blocks
        contact_mask = generate_mask[row] | generate_mask[col]
        row, col = row[contact_mask], col[contact_mask]
        
        if len(row) == 0:
            return 0
        
        # Calculate distances
        dists_gt = torch.norm(block_centers_gt[row] - block_centers_gt[col], dim=-1)
        
        # Define contacts as blocks within threshold distance
        contact_mask = dists_gt < contact_th
        row, col = row[contact_mask], col[contact_mask]
        
        if len(row) == 0:
            return 0
            
        contact_dists_gt = dists_gt[contact_mask]
    
    # Calculate predicted block centers and distances
    block_centers_t = scatter_mean(X_t, block_ids, dim=0, dim_size=block_ids.max() + 1)
    contact_dists_t = torch.norm(block_centers_t[row] - block_centers_t[col], dim=-1)
    
    # Loss for contact distance preservation
    loss = F.smooth_l1_loss(contact_dists_gt, contact_dists_t, reduction='none')
    
    # Weight by importance (closer contacts are more important)
    weights = torch.exp(-contact_dists_gt / 4.0)
    loss = loss * weights
    
    # Only apply loss for early timesteps
    # Get timestep for each block by finding representative atoms
    if len(row) > 0:
        # Get the first atom index for each block
        block_to_first_atom = scatter_min(torch.arange(len(block_ids), device=block_ids.device), block_ids, dim=0)[1]
        # Get timesteps for each block using their first atoms
        block_timesteps = t[block_to_first_atom]
        
        # Get timesteps for the contact pairs
        center_t = (block_timesteps[row] + block_timesteps[col]) / 2
        loss = loss[center_t < t_th]
    
    return loss.mean() if len(loss) else 0

def _angle_preservation_loss(X_gt, X_t, t, bonds, block_ids, generate_mask, t_th=0.25):
    """Preserve bond angles for proper molecular geometry"""
    with torch.no_grad():
        generate_mask = generate_mask[block_ids]
        bond_mask = generate_mask[bonds[:, 0]] & generate_mask[bonds[:, 1]]
        bonds = bonds[bond_mask]
        
        # Find angle triplets: atoms connected by two consecutive bonds
        bond_dict = {}
        for i, (src, dst, _) in enumerate(bonds):
            src, dst = src.item(), dst.item()
            if src not in bond_dict: bond_dict[src] = []
            if dst not in bond_dict: bond_dict[dst] = []
            bond_dict[src].append(dst)
            bond_dict[dst].append(src)
        
        angle_triplets = []
        for center, neighbors in bond_dict.items():
            if len(neighbors) >= 2:
                for i in range(len(neighbors)):
                    for j in range(i+1, len(neighbors)):
                        angle_triplets.append([neighbors[i], center, neighbors[j]])
        
        if not angle_triplets:
            return 0
            
        angle_triplets = torch.tensor(angle_triplets, device=X_gt.device)
        
        # Calculate ground truth angles
        v1 = X_gt[angle_triplets[:, 0]] - X_gt[angle_triplets[:, 1]]
        v2 = X_gt[angle_triplets[:, 2]] - X_gt[angle_triplets[:, 1]]
        v1_norm = F.normalize(v1, dim=-1)
        v2_norm = F.normalize(v2, dim=-1)
        cos_angles_gt = torch.clamp((v1_norm * v2_norm).sum(-1), -1, 1)
    
    # Calculate predicted angles
    v1_t = X_t[angle_triplets[:, 0]] - X_t[angle_triplets[:, 1]]
    v2_t = X_t[angle_triplets[:, 2]] - X_t[angle_triplets[:, 1]]
    v1_t_norm = F.normalize(v1_t, dim=-1)
    v2_t_norm = F.normalize(v2_t, dim=-1)
    cos_angles_t = torch.clamp((v1_t_norm * v2_t_norm).sum(-1), -1, 1)
    
    # Angular loss
    loss = F.smooth_l1_loss(cos_angles_gt, cos_angles_t, reduction='none')
    
    # Only apply loss for early timesteps
    center_t = t[angle_triplets[:, 1]]
    loss = loss[center_t < t_th]
    return loss.mean() if len(loss) else 0
...
class CondIterAutoEncoder(nn.Module):
    def __init__(
            ...
            loss_weights = {
                ...
                'angle_preservation_loss': 0.2,
                'contact_preservation_loss': 0.1,
            },
    ...
    def forward(
        ...
        loss_dict = {
            ...
            'angle_preservation_loss': _angle_preservation_loss(X_gt, X_final, t, bonds, block_ids, generate_mask),
            'contact_preservation_loss': _contact_preservation_loss(X_gt, X_final, t, bonds, block_ids, generate_mask),
        }
