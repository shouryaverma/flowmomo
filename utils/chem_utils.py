...
MAX_VALENCE = {
    'B': 3, 'Br': 1, 'C': 4, 'Cl': 1, 'F': 1, 'I': 1, 
    'N': 3, 'O': 2, 'P': 5, 'S': 6,
    # Common ionic species (typically don't form covalent bonds in organic contexts)
    'Na': 0, 'K': 0, 'Ca': 0, 'Mg': 0, 'Li': 0,
    # Other common atoms in biological systems
    'Se': 4, 'Si': 4, 'Zn': 0, 'Fe': 0, 'Mn': 0
}
...
def get_submol_atom_map(mol, submol, group, kekulize=False):
    ...
    # First try direct substructure matching
    matches = mol.GetSubstructMatches(submol)
    old2new = { i: 0 for i in group }  # old atom idx to new atom idx
    found = False
    ...
    # If direct matching fails, try with SMILES canonicalization
    try:
        # turn to smiles order
        smi = mol2smi(submol)
    ...
    # If still not found, try with different kekulization
    try:
        smi = mol2smi(submol)
        submol_kekule = smi2mol(smi, kekulize=(not kekulize), sanitize=False)
        
        matches = mol.GetSubstructMatches(submol_kekule)
        old2new = { i: 0 for i in group }  # old atom idx to new atom idx
        found = False
    ...
    # If all matching attempts fail, create a simple sequential mapping
    # This is a fallback to prevent crashes
    print(f"WARNING: Substructure matching failed for group {group}, using sequential mapping")
    return {group[i]: i for i in range(len(group))}
...
def connect_fragments(mol, atom_coords):
    ...
    for frag_idx, frag in enumerate(fragments):
        for atom_idx in frag:
            ...
            if atom.GetSymbol() not in MAX_VALENCE:
                continue
